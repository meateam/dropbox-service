...
export class DropboxMethods {
  ...
  static async GetTransfersInfo(
  ...
  ): Promise<{ transfersInfo: ITransferInfo[] }> {
    ...
    const paginatedTransfers: IPaginatedTransfer[] = await TransferRepository.getMany(
      partialFilter
    );
  ...
  static async GetApproverInfo(
    ...
    const approverInfo: IApproverInfo = await approvalService.getApproverInfo(
      id,
      destination
    );
    return approverInfo;
  ...
  static async CreateRequest(call: grpc.ServerUnaryCall<any>) {
    ...
    if (destUsers.length < 1)
      throw new ClientError('must require at least 1 dest user');
    await Promise.all(
      destUsers.map(async (destUser: IApprovalUser) => {
        try {
          ...
          throw new ClientError(
            `cant get user: ${destUser.id} with destination: ${destination}`
          );
        }
      })
    );
